...
async def test_keyless_url_stays_on_compatibility_path(hass) -> None:
    """An ambiguous keyless v1 URL can still be a Supervisor endpoint."""
    entry = MockConfigEntry(
        domain=DOMAIN,
    ...
    assert CONF_AUTH_SOURCE not in entry.data
    assert entry.unique_id == FAKE_AUTH_URL
...
async def test_default_entry_migrates_alongside_manual_entry(hass) -> None:
    """Mixed legacy entries do not depend on a branch-local import."""
    manual = MockConfigEntry(
        domain=DOMAIN,
        data={CONF_AUTH_URL: FAKE_AUTH_URL},
        unique_id=FAKE_AUTH_URL,
        version=1,
    )
    ...
    default.add_to_hass(hass)

    assert await familylink.async_migrate_entry(hass, default)

    assert default.version == 2
    assert default.data[CONF_AUTH_SOURCE] == AUTH_SOURCE_MANAGED
    assert default.unique_id == "familylink_default"

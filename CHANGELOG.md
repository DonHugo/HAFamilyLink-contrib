...
- **Auth files now use fail-closed persistence.** A validated, non-symlink `/share/familylink` directory is pinned and locked across cooperating access; its `.key`, `cookies.enc`, and generated `api_key` children reject symlink or special-file substitution, enforce private modes, bound reads, and use durable race-safe atomic creation or replacement. Corrupt cookies are retained for safe replacement after reauthentication rather than removed through a cleanup race. Existing encrypted cookies, explicit `API_KEY` precedence, and the standalone no-key policy remain compatible.

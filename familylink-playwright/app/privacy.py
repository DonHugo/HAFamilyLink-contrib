"""Privacy-safe logging for the Family Link authentication service."""
...
_OPERATION_TOKENS = tuple((operation, operation) for operation in _OPERATIONS)
_STATUS_TOKENS = (
    (("fail", "error", "invalid", "denied"), "failed"),
    (("skip", "missing", "not found", "timeout"), "unavailable"),
    (("success", "saved", "loaded", "generated", "completed"), "succeeded"),
)
...
class PrivacyLogger:
    ...
    def _message(message: object, level: int) -> str:
        ...
        return f"Family Link auth operation={operation} status={status}{suffix}"
    ...
    def event(
        ...
        *,
        ...
        safe_operation = operation if operation in _OPERATIONS else "auth_service"
        ...
        self.__logger.log(
            level,
            f"Family Link auth operation={safe_operation} status={safe_status}{suffix}",
        )
...
def get_privacy_logger(name: str) -> PrivacyLogger:
    """Return a logger that cannot emit authentication data."""

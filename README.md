...
Family Link logs contain only allow-listed operation/status metadata and safe
HTTP status codes. Dynamic entity attributes are excluded from new Home
Assistant Recorder rows. Primary entity states and all live attributes remain
available, so dashboards, services, strict mode, and events continue to work.
